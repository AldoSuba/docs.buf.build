...
Connect-Web is a small library to call remote procedures from a web browser.
Unlike REST, you get a type-safe client and never have to think about
serialization again.

In this guide, we'll use Connect-Web to create a web interface for [ELIZA](https://en.wikipedia.org/wiki/ELIZA),
...
a Protocol Buffer schema we'll use to generate a Connect-Web client.
...
use [remote packages](https://docs.buf.build/bsr/remote-packages/overview), a feature of the Buf Schema Registry. The 
...
generation after the tutorial, in the page [Generating code](#)).
...
the user interface, but Connect-Web is framework-agnostic and even works with
vanilla Javascript.
