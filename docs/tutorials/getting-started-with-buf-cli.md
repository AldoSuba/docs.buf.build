...
* Create & Configure a Buf Module
* Generate Code using Remote Plugins
* Lint & format your API
...
specify options for each plugin independently.
...
- It executes the [`bufbuild/connect-go`](https://buf.build/bufbuild/connect-go) plugin to generates client and server
  stubs for connect-go. Compatible with the gRPC, gRPC-Web, and Connect RPC protocols into the `gen` directory. Connect
  is an RPC protocol which supports gRPC — including streaming! They interoperate seamlessly with Envoy, grpcurl, gRPC
  Gateway, and every other gRPC implementation. Connect servers handle gRPC-Web requests natively, without a translating
  proxy. Learn more [here](https://connect.build/).
...
downloading, or running [plugins][plugins] on your local machine. If you want to use local plugins you absolutely can,
...
* [Getting Started with Buf Studio](#)
