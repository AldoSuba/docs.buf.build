...
[module](/bsr/explanation.mdx#modules) you pushed to the
[BSR](/bsr/explanation.mdx) to implement a gRPC client and server in Go. That's a
...
`PetStoreService` API and use a [workspace](/buf/other/workspaces.mdx) to
...
[`PACKAGE_VERSION_SUFFIX`](/buf/lint/reference/rules.md#package_version_suffix), such as the
