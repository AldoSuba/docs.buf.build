...
talk in terms of `FileDescriptorSets`.
...
**[Modules](../bsr/overview.md#modules) are the primitive of Buf, and Buf images
represent the compiled artifact of a module.** In fact, images contain
information about the module used to create it, which powers a variety of
[BSR](../bsr/overview.md) features. For clarity, the [`Image`][image-proto]
...
[`FileDescriptorSet`s][filescriptorset] are images, we can use`protoc` output as
`buf` input. Here's an example for [`buf lint`](../lint/usage.md):
...
Since `buf` "understands" [`FileDescriptorSet`s][filedescriptorset], we can
