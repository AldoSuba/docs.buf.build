...
> By default, `buf` uses the current directory as its schema input for all commands.
...
- A **schema** is either a **source**&mdash;a set of `.proto` files&mdash;or an
  **image**&mdash;a set of `.proto` files built into a single, encapsulating
  Protobuf [`Image`][image-proto] message.
- All **schemas** have a **format** that describes the type of the **schema**.
  Commonly used formats include [`dir`](#dir) and [`git`](#git). The **format**
  of an **schema** is usually derived automatically but you can opt to set it
  explicitly.
...
With the BSR, you can refer to any version of a Buf module and use it as a
schema for each of the `buf` commands. You can lint all the Protobuf files in the
...
specified as the first argument. Schemas are specified as a string and have this
structure:
...
The path specifies the path to the schema. The options specify options to
interpret the schema at the path.
...
its schema input for all commands.
