...
[build](/buf/build/how-to), [lint](/buf/lint/explanation), and
[breaking change](/buf/breaking/explanation) errors which integrate with IDEs, scripts,
and other tools.
...
To use Vim integration `buf` must be [installed](installation). Using
...
as part of your [CI](/buf/ci-cd/how-to) process.
...
You need to have `buf` [installed](installation) to use it.
...
Make sure `buf` is [installed](installation) on your PATH, then configure
