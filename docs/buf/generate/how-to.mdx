---
id: how-to
title: "Master Buf Generate: A guide to transform development"
description: "Unlock the power of Buf Generate: A step-by-step guide to revolutionize your development process"
---

> We highly recommend completing [the tour](/tutorials/tour/configure-and-build.md) to get an
...
[input](../other/inputs). The `buf.gen.yaml` template lists one or more
plugins and, optionally, other file option configurations with
[managed mode](managed-mode/reference). For more information on the `buf.gen.yaml`
configuration, see the [reference](../../configuration/v1/buf-gen-yaml.md).

## Define a module

To get started, create a [module](../../bsr/explanation#modules) by adding a
...
Now that you have an [input](../other/inputs) to generate code for, we
...
Run this to generate from the [input](../other/inputs) in your current
directory:
...
[**Remote plugins**](../../bsr/remote-plugins/how-to) make it
...
You can generate from a Buf [module](../../bsr/explanation#modules) on the
[Buf Schema Registry](../../bsr/introduction) (BSR) by providing the module name
as the [input](../other/inputs):
...
> [Buf input format documentation](../other/inputs#source-formats).
...
templates for the same [input](../other/inputs), we recommend using
...
[Bazel](../other/bazel.md) integration - it is better to let `buf`
...
1. Use [remote plugins](../../bsr/remote-packages/explanation#remote-plugins) to
generate code without requiring local plugins.
1. Use [generated assets](../../bsr/remote-packages/explanation) to fetch
