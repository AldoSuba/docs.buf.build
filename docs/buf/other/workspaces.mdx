...
[modules](../explanation.mdx#modules) to import Protobuf files from other local
...
[BSR](../explanation.mdx) so that the other module can update its dependency
...
from the local [module cache](../explanation.mdx#module-cache). This requires
that the latest change has been pushed to the [BSR](../explanation.mdx) and
