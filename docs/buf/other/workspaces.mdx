...
[modules](../../bsr/explanation#modules) to import Protobuf files from other local
...
[BSR](../../bsr/explanation) so that the other module can update its dependency
...
[input](../other/inputs) that contains a `buf.work.yaml`, each of the
modules contained within the workspace is linted. Other commands, such as
`buf build`, merge workspace modules into one, so that all the files
contained are consolidated into a single [image](../../bsr/data-model/images.md).
...
from the local [module cache](../../bsr/explanation#module-cache). This requires
that the latest change has been pushed to the [BSR](../../bsr/explanation) and
