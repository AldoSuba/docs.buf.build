...
[`buf lint`](./usage.md) command. When you run `buf lint`, `buf` runs a set of
...
- **Editor integration**. The default error output is easily parsed by most
  ...
  provide [Vim and Visual Studio Code integration](../editor-integration.md) but
  we may support other editors in the future, such as Emacs and IntelliJ IDEs.
