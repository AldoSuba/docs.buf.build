...
[`buf.yaml`](../configuration/v1/buf-gen-yaml.md) file, which is placed at the root
...
<Syntax
  ...
  segments={[
    ...
    { separator: "/" },
    { label: "repository", kind: "variable", href: "/bsr/explanation#modules" }
  ]}
/>
...
For more information, see [Generated documentation](documentation/explanation).
...
> See the [Usage](reference#add-a-dependency) section for a detailed
> example.
...
[workspace](../buf/other/workspaces.mdx) to discover modules through your file
system. If you are in a workspace, `buf` looks for `deps` in your
[workspace configuration](../buf/other/workspaces.mdx#configuration) _before_
...
> For an in-depth example check out the [Tour - Use a Workspace](/tutorials/tour/use-a-workspace)
...
see the [remote plugins'](remote-plugins/how-to) documentation.
...
[managed mode](../buf/generate/managed-mode/reference) as it relates to code generation.
...
[remote-plugins]   remote-plugins/how-to
