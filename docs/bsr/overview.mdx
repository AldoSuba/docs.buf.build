...
module to a specific version, you can do so by specifying tag, commit, or draft
in your `deps` after the `:` delimiter. Ideally, authors keeps modules
...
**Draft**: A temporary commit in development workflow with a human readable
...
be replaced by another commit with the same draft name and is deletable. When it
...
The Buf team will package and distribute plugins to the BSR, browsable at:
...
To learn more about how hosted plugins can be used in code generation workflows,
see the

[Remote plugin execution](/bsr/remote-generation/remote-plugin-execution.mdx)
...
For more information, see the [Generated
SDKs][remote-generation-overview-generated-sdks] documentation.
