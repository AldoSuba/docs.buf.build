...
Many organizations with public Protobuf files are already using the BSR, and some bigger ones are officially maintained by Buf. These include
...
> See the [Usage](../bsr/usage.mdx#add-a-dependency) section for a detailed example.
...
If you want to depend on local modules, you can set up a [workspace](../reference/workspaces.mdx) to discover modules through your file system. If you are in a workspace, `buf` looks for `deps` in your [workspace configuration](../reference/workspaces.mdx#configuration) _before_ attempting to find it on the BSR.
