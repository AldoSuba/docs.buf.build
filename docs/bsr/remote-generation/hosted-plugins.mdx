...
> [Key concepts documentation](overview.md#concepts).
...
It is possible to reference both local and hosted plugins within a single
template file. The `buf generate` command issues an RPC to the BSR to execute
the hosted plugins against the given input. Once execution is finished the
