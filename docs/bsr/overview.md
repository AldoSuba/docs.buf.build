...
**Draft**: A temporary commit in development workflow with a human readable name, similar to a Git feature branch but without history. It is useful for developing on the module and want to commit outside the main branch. Can be replaced by another commit with the same draft name and is deletable.
